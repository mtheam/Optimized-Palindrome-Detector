...
int endOfFile = 0; //1 if EOF reached
...
// takes a line and returns the longest palindrome on standard input
...
//    i.e. ' ', '\n', '\r', '\t', '\n', '\f'
std::string longestPalindrome(string inputLine)
{
  string maxPali = "";
  //cout << "determining longest palindrome" << endl; 

  if( inputLine.size() == 0) return maxPali;
  auto words = split(inputLine);
  for( auto word : words) {
